...
values (2, 1, 10, now());
insert into order_composite_pk
values (2, 1, default, now());
-- >단점 : pr가 길고, 다른 테이블에서 해당 주문을 참조하는 경우 FK도 3개 컬럼이 필요함
...
-- [23000][1452] Cannot add or update a child row: a foreign key constraint fails (`menudb`.`address`, CONSTRAINT `address_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`))

-- 부모 행(user) 삭제
delete from user where id = 1;
-- [23000][1451] Cannot delete or update a parent row: a foreign key constraint fails (`menudb`.`address`, CONSTRAINT `address_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`))
...
-- 컬럼추가
-- price 정수타입(int) 기본값 0, 코멘트를 '가격', null 이용 불가
    alter table product add price INT NOT NULL DEFAULT 0 COMMENT '가격'; -- 순서 [first] [after 기존컬럼]
...
-- 컬럼명 변경
alter table product change column name prod_name varchar(100) not null;