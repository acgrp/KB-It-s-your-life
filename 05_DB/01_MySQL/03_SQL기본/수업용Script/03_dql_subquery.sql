...
from tbl_menu
where category_code
          in (select c.category_code
              from tbl_category c
              where c.ref_category_code =
                    (select tc.category_code
                     from tbl_category tc
                     where tc.category_name = '식사'));
...
select menu_code,
       ...
       menu_price,
       category_code,
       (select category_name from tbl_category c where t.category_code = c.category_code) category_name
from tbl_menu t;
...
from (select menu_code,
             ...
             category_code,
             (select category_name from tbl_category c where t.category_code = c.category_code) category_name
      from tbl_menu t) m -- 인라인뷰(테이블) 별칭 작성
where menu_code = 1;

# CTE
with my_menu as ( select menu_code 메뉴코드,
                         menu_name,
                         menu_price,
                         category_code,
                         (select category_name from tbl_category c where t.category_code = c.category_code) category_name
