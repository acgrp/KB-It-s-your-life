...
select instr('사과딸기바나나', '딸기'), instr('사과딸기바나나', '키위');
...
select regexp_replace('123,456원', '[^0-9]', ''),
...
# 숫자처리함수
select ceiling(1234.56), -- 올림
       floor(1234.56), -- 내림
       truncate(1243.56, 1), -- 절삭
       round(1234.56),
       round(1234.56, 2)
;
...
# adddate(date, 일수)
# subdate(date, 일수)
select now(), adddate(now(), 1), subdate(now(), interval  10 year) -- 시간 자료형 day/month/tear
;

# extract(단위 from datetime) -> 숫자
select extract(year from now()), -- 년
