...
select * from tbl_menu m
                  join tbl_category c on m.category_code = c.category_code; -- 21행
...
select * from tbl_menu m
                  left join tbl_category c on m.category_code = c.category_code; -- 22행

select * from tbl_menu m
                  right join tbl_category c on m.category_code = c.category_code; -- 25행

# cross join
# - 카테시안곱. 모든 경우의 수를 조인처리(조건이 필요없음)
select * from tbl_menu
cross join tbl_category;  -- 264행 = 22 x 12

# self join
# - 하나의 테이블안에서 한행이 다른행을 참조하는 관계가 있는 경우, 같은 테이블끼리 조인가능
select * from tbl_category;

SELECT concat(t.category_name, ' > ', c.category_name) from tbl_category t
join tbl_category c on t.category_code = c.ref_category_code;
