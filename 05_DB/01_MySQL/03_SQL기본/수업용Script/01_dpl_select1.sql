...
-- 계산된 식 사용 가능
-- 별칭(alias)지정 가능
select menu_name, menu_price 원가, menu_price *0.9 '할인된 금액' from tbl_menu;
...
# - concat(문자열, 문자열)
-- 000메뉴의 금액은 000원 입니다.
select concat(menu_name, '메뉴의 금액은 ', menu_price, '원 입니다.') 메뉴설명 from tbl_menu;
...
select * from tbl_menu
where
    # cmenu_price < 10000 or menu_price > 20000
...
select * from tbl_menu
where menu_name like '___쥬스'; -- 3글자 + 쥬스
    # menu_name like '%마늘%'; -- 마늘이 들어가는 메뉴명
    #menu_name like '%밥'; -- 밥으로 끝나는 메뉴 조회


# 특정목록에 포함여부 in
-- 카테고리 코드 중 8 이거나 , 9 또는 10 만 조회
select * from tbl_menu where category_code in (8, 9, 10);
