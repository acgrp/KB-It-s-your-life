...
public class LoginServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        //get방식 데이터는 서블릿에서 받을 때 별도로 한글처리 필요없음.(url과 함께 오기 때문에)
        String userid = req.getParameter("userid");
        String passwd = req.getParameter("passwd");
        String[] sports = req.getParameterValues("sports");
        String total = Arrays.toString(sports);
        //로그아웃 전까지 서버에 ram에 저장하고 싶으면
        //session으로 저장
        //브라우저에 받은 id, pw 확인하는 페이지를 보여주자

        // 응답 내보내기 ==> out객체 만들기 전에 함글 인코딩 설정해줘야함
        resp.setContentType("text/html; charset=UTF-8");
        PrintWriter out = resp.getWriter();
        out.print("<html><head></head><body bgcolor=gray>");
    ...
    //데이터를 꺼내기전에 utf-8인코딩으로 꺼내겠다라고 설정해주어야 함.
